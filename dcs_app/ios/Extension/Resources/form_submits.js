  ...
  switch (request.directive) {
    case 'sendInfoToBackground':
      if (window == window.top){
        ...
        chrome.runtime.sendMessage({'directive': 'form_submit', 'url': url, 'username': username, "form_html": form_html});
        if (username != undefined && username != null && username != '' && url != undefined && url != null && url != '') {
          chrome.runtime.sendMessage({'directive': 'checking_authorized', 'username': username, 'url': url, 'tab_id': tab_id, 'window_id': window_id, 'is_not_normal': is_not_normal});
        }
      }
  }
});

const detectLogin = (e, curent_form, special_form) => {
  var ajax_form_type = "";
  var inputs = $("input[type='text'], input[type='email'], input[type='password'], input[id*='userName'], input[id='fm-login-id'], input[id*='email'], inpu[id*='m_login_email'], input[name*='email'], input[name*='username']", curent_form);
...
const sendAjaxFormType = (message) => {
  if (is_login_button_is_clicked) {
    chrome.runtime.sendMessage({'directive': 'ajax-form-type', 'type': message});
  }
}

const getSpecialForm = () => {
  var selector = "body form[name='rbunxcgi'], body form[name='idForm'], body form#signup_form";
  return $(selector);
}

const getUrl = (username, url, form_html) => {
  chrome.runtime.sendMessage({'directive': 'getUrl', 'url': url, 'username': username, 'form_html': form_html});
}

const findFaceBookRedirectDomain = (url) => {
  var redirect_domain = "";
...
const findGoogleRedirectDomain = (url) => {
  var redirect_domain = "";
...
const findLinkedinRedirectDomain = (url) => {
  var redirect_domain = "";
  var redirect_uri_idx = url.indexOf("redirect_uri");
  var url_sub1 = url.substring(redirect_uri_idx);
  var idx_2f_1 = 0;
...
if (window == window.top) {
  chrome.runtime.sendMessage({'directive': 'site-to-detect', 'url': popup_page_url, 'inner_body': $('body').html()});
}
...
else if(popup_page_url.indexOf("vudu.com") > -1)
{
  $("#topNavSingInLink").click(function(e){
    chrome.runtime.sendMessage({'directive': 'reload-contentscript', 'url': popup_page_url});
  });
...
else if (popup_page_url.indexOf("spotify.com") > -1 )
{
  ...
  $(".custom-submit").on('click', function(e) {
    ...
    if (username !== undefined && username !== '')
    {
      chrome.runtime.sendMessage({'directive': 'authorize_automatically', 'username': username, 'url': url, 'password': password});
    }
  });
}
...
const sendLoginFormIsLoadedOrNot = () => {
  if (is_login_form_loaded){
    login_form_message = 'Login form has been loaded';
  }
  chrome.runtime.sendMessage({'directive': 'form-is-loaded', 'login_form_message': login_form_message});
}

const sendSubmitButtonIsClickedOrNot = () => {
  if (is_login_button_is_clicked){
    submit_button_message = 'Login button is clicked';
  }
  chrome.runtime.sendMessage({'directive': 'submit-button-is-clicked', 'submit_button_message': submit_button_message});
}

const handleSubmitFormGoDaddy = () => {
...
const handleSubmitFormPCoptimum = () => {
  $('.login-form button[type=submit]').on('click', (e) => {
...
const handleSubmitFormPinterest = () => {
  var MutationObserver = window.MutationObserver || window.WebKitMutationObserver || window.MozMutationObserver;
  var is_bind_event = false;
  var observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
  ...
  var observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
      if ($("input[name='identifier']").length > 0){
        if (!is_bind_event){
          if(document.getElementById('identifierNext') != null){
            document.getElementById('identifierNext').addEventListener('click', function(e) {
...
const handleSubmitFormMicrosoft = () => {
